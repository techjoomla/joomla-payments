...
class PlgPaymentEpaydk extends CMSPlugin
{
	 ...
	 * @return   string  vars
	 */
	public function buildLayout($data, $layout = 'default')
	{
	...
	public function onTP_Processpayment($data, $vars)
	{
		...
		if ($isValid)
		{
			// Epay uses minor values
			$mc_gross = floatval($data['amount'] / 100);
		}
		...
		if ($isValid)
		{
			...
			// Remove 0 eg if return currency=036
			$mc_currency    = (int) $data['currency'];
			/*$currency = strtoupper(AkeebasubsHelperCparams::getParam('currency','EUR'));*/

			// Remove 0 eg if return currency=036
			$mc_currencyINT = (int) $data['currency'];
	 ...
	 * @return  array.
	 */
	public function getOnlyResponseData($data)
	{
		$remvoveData = array(
			'option',
			"controller",
	...
	private function isValidRequest($data)
	{
		...
		if ($this->params->get('md5', 0))
		{
			...
			/*md5(implode("", array_values($params)) . "SecretMD5Key");*/
